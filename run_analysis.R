...
#First we create the final dataset

finalDF <- rbind(train, test)

#We add an "Activity" and "Subject Number" Column to the dataset.
#We first melt the dataframe and then set it with the dcast() function
finalDF[["Activty"]] <- factor(finalDF[, Activity], levels = activitylabels[["ClassLabels"]], labels = activitylabels[["ActivityName"]])
finalDF[["SubjectNumber"]] <- as.factor(finalDF[, SubjectNumber])

finalDF2 <- reshape2::melt(data = finalDF, id = c("SubjectNumber", "Activity"))
finalDF3 <- reshape2::dcast(data = finalDF2, SubjectNumber + Activity ~ variable, fun.aggregate = mean)
finalDF3



data.table::fwrite(x = finalDF3, file = "cleanData.csv", quote = FALSE)
